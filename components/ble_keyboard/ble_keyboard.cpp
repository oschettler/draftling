 ...
 *
 * Uses NimBLE for BLE scanning and the ESP-IDF esp_hidh component for
 * HID-over-GATT host.  ESP32-S3 does not support Bluetooth Classic,
 * so only BLE is used.
 *
    ...
    case BLE_GAP_EVENT_DISC: {
        ...
        if (is_kbd && !s_kbd_found) {
            ...
            if (fields.name_len > 0 && fields.name != NULL) {
                ...
                memcpy(s_dev_name, fields.name, len);
                s_dev_name[len] = '\0';
            }
...
{
    if (!s_connected && s_scan_task_handle == NULL) {
